 ...
 */
const fs=require('fs');
const words=JSON.parse(fs.readFileSync('src/assets/words/words.json','utf8'));
const re=/^[a-zçğıöşü]+$/;
...
// tüm başlangıç adayları (uzunluk 8→4), 3'e ulaşanlar
let starts=[];
for(let L=8;L>=4;L--){ if(!byLen[L])continue; for(const s in byLen[L]) for(const w of byLen[L][s]) if(canReach(w)) starts.push(w); }
console.log('3\'e inebilen başlangıç kelimesi:', starts.length);

// piramit havuzu üret: çeşitlilik için tepe(3-harf) başına sınır ve tam-küme tekrarını engelle
shuffle(starts);
starts.sort((a,b)=>b.length-a.length); // önce yüksek piramitler
const pool=[]; const seenSet=new Set(); const topCount={};
for(const w of starts){
  const chain=buildChain(w); if(!chain)continue;
  const key=chain.slice().sort().join('|'); if(seenSet.has(key))continue;
  const top=chain[chain.length-1]; if((topCount[top]||0)>=3)continue; // aynı tepeden en çok 3
  seenSet.add(key); topCount[top]=(topCount[top]||0)+1;
